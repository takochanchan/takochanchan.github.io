...
test("catalogue metadata is complete and unique", () => {
  assert.equal(publications.length, 385);
...
test("split volumes share one canonical bibliography record", () => {
  ...
  assert.equal(cataloguePublications.length, 375);
  assert.equal(majorCataloguePublications.length, 184);
...
test("full-text search assignments stay inside stable Pages shards", async () => {
  ...
  assert.equal(counts.get("002"), 108);
  assert.equal(
...
test("short works use explicit author groups instead of page-count rules", () => {
  assert.equal(majorPublications.length, 194);
...
test("home page contains scalable archive controls", async () => {
  ...
  assert.match(
    html,
    /bibliographicCounts:\{"books":184,"papers":191\}/,
  );
  ...
  assert.match(html, /id="book-match-count">184<\/strong>件/);
...
test("local covers and release assets match the recorded manifest", async () => {
  ...
  for (const asset of manifest.assets.filter((item) =>
    /cover(?:-[a-z0-9]+)*\.(?:jpg|png|svg)$/.test(item.path)
  )) {
...
test("repository source contains covers but no PDF, EPUB, or split parts", async () => {
  ...
  assert.equal(
    files.filter((file) => /cover(?:-[a-z0-9]+)*\.(?:jpg|png|svg)$/.test(file)).length,
    publications.length,
  );
  assert.equal(
...
test("Galvao 1563 uses the approved Portuguese cover label and BNP source", () => {
  const item = publications.find((item) => item.slug === "galvao-tratado-descobrimentos-1563");
  assert.ok(item);
  assert.equal(item.title, "発見誌");
  assert.equal(item.pageCount, 109);
  assert.equal(item.searchShard, "002");
  assert.equal(item.series, "NAVEGAÇÕES E DESCOBRIMENTOS · 1563");
  assert.equal(item.sourceUrl, "https://purl.pt/15321");
  assert.match(item.rights, /Public Domain Mark/);
  assert.equal(item.cover, "publications/galvao-tratado-descobrimentos-1563/cover.jpg");
});



test("Serrano 1911 preserves the approved title and article classification", () => {
  const item = publications.find(item => item.slug === "serrano-archivo-indias-panama-1911");
  assert.equal(item.title, "インディアス文書館とパナマ地峡の探検 1527-1534年");
  assert.equal(item.recordClass, "short-work");
  assert.equal(item.authorKey, "manuel-serrano-y-sanz");
  assert.equal(item.pageCount, 48);
  assert.equal(item.searchShard, "002");
});
