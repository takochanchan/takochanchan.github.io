...
test("home page contains scalable archive controls", async () => {
  ...
  assert.match(html, /\/archive\.css\?v=20260811-fulltext-search-v4/);
  assert.match(html, /\/archive\.js\?v=20260811-fulltext-search-v4/);
  assert.match(html, /\/fulltext-search\.css\?v=20260811-fulltext-search-v4/);
  assert.match(html, /\/fulltext-search\.js\?v=20260811-fulltext-search-v4/);
...
test("every publication has a detail page, local cover, and release links", async () => {
  for (const item of publications) {
    ...
    if (item.recordClass === "short-work") {
      assert.match(
        html,
        /href="\/\?v=20260811-fulltext-search-v4#short-works">← 論文へ戻る<\/a>/,
      );
    } else {
      assert.match(
        html,
        /href="\/\?v=20260811-fulltext-search-v4#publications">← 書籍へ戻る<\/a>/,
      );
    }
    for (const label of [
      "底本",
