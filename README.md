...
トップページは「刊本・大部論文」と「短篇論文・報告」に分かれています。
短篇は頁数ではなく初出形態を確認して明示的に分類し、同ファイルの
`shortWorkAuthorBySlug` にslugと安定した著者キーを追加します。同じ著者キーを
持つ短篇は、著者見出しの下へ刊行年順で自動的に積み上がります。短い単独刊本は
この指定を行わず、刊本・大部論文側に残します。
...
「刊本・大部論文」セクションには、一覧の書誌検索とは別に「Googleでサイト内を検索」を設けています。
