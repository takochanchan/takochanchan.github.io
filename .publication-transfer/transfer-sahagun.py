...
request=json.loads(pathlib.Path('.publication-upload-requests/sahagun-20260909-067.json').read_text())
...
with tempfile.TemporaryDirectory() as temp:
 ...
 for name,a in expected.items():
  replace=False
  if name in existing:
   olddir=temp/'existing';olddir.mkdir(exist_ok=True)
   subprocess.run(['gh','release','download','publications-current','--repo',repo,'--pattern',name,'--dir',str(olddir)],check=True)
   old=(olddir/name).read_bytes()
   if digest(old)!=a['sha256']:
    before=next(x for x in request['previous_assets'] if x['filename']==name)
    assert len(old)==before['size'] and digest(old)==before['sha256'],'Concurrent asset revision: '+name
    replace=True
  if name not in existing or replace:
   subprocess.run(['gh','release','upload','publications-current',str(temp/name),'--repo',repo,'--clobber'],check=True)
 ...
 subprocess.run(['gh','api','--method','DELETE','repos/'+repo+'/git/refs/heads/agent%2Fsahagun-assets-20260909-067'],check=True)
